...
from src.utils.improved_logging import StructuredLogger
...
class TestPipelineHelpers:
    ...
    def test_setup_directories_production_vs_test(self, pipeline):
        ...
        assert pipeline.archive_manager.archive_current_analysis.call_args[1]['run_type'] == 'test'
        
        # Production mode
        pipeline._setup_directories("test-idea", debug=False)
        assert pipeline.archive_manager.archive_current_analysis.call_args[1]['run_type'] == 'production'
    ...
    def test_find_feedback_file_fallback_to_main(self, pipeline, temp_dir):
        ...
        mock_logger = Mock()
        result = pipeline._find_feedback_file(
            iterations_dir, iteration_count=2, analysis_dir=analysis_dir, logger=mock_logger
        )
    ...
    def test_save_analysis_files_creates_both_files(self, pipeline, temp_dir):
        ...
        result = pipeline._save_analysis_files(
            test_content, iteration_count=1, 
            analysis_dir=analysis_dir, iterations_dir=iterations_dir,
            logger=mock_logger
        )
